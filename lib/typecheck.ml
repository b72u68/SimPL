...
let typ_mismatch_err t1 t2 =
  Printf.sprintf "expected type %s, got %s" (string_of_type t1)
    (string_of_type t2)

let typ_err (spos, epos) msg =
  let err_msg =
    Printf.sprintf "%s--%s: %s" (string_of_pos spos) (string_of_pos epos) msg
  in
...
and typecheck_exp ctx exp =
  ...
  | EArr arr ->
      if List.length arr = 0 then
        typ_err exp.eloc "empty array, expected at least 1 element in an array"
      else
  ...
  | EArrIdx (v, e) -> (
      ...
      | TArray t ->
          ...
          if not (teq e_type TInt) then
            typ_err e.eloc (typ_mismatch_err TInt e_type)
          else t
      | _ ->
          typ_err exp.eloc
            (Printf.sprintf "expected type array, got %s"
               (string_of_type v_type)))
  | EBinop (bop, e1, e2) -> (
      let e1_type = typecheck_exp ctx e1 in
      let e2_type = typecheck_exp ctx e2 in
      match bop with
      | Plus | Minus | Times | Div ->
          if not (teq e1_type TInt) then
            typ_err e1.eloc (typ_mismatch_err TInt e1_type)
          else if not (teq e2_type TInt) then
            typ_err e2.eloc (typ_mismatch_err TInt e2_type)
          else TInt
      | Le | Lt | Ge | Gt ->
          if not (teq e1_type TInt) then
            typ_err e1.eloc (typ_mismatch_err TInt e1_type)
      ...
      | And | Or ->
          if not (teq e1_type TBool) then
            typ_err e1.eloc (typ_mismatch_err TBool e1_type)
          else if not (teq e2_type TBool) then
            typ_err e2.eloc (typ_mismatch_err TBool e2_type)
          else TBool
      | Eq | Neq ->
          if not (teq e1_type e2_type) then
            typ_err e2.eloc (typ_mismatch_err e1_type e2_type)
          else TBool)
  | EIf (e1, e2, e3) ->
      let e1_type = typecheck_exp ctx e1 in
      if not (teq e1_type TBool) then
        ...
        if not (teq e2_type e3_type) then
          typ_err e3.eloc (typ_mismatch_err e2_type e3_type)
        else e2_type
  | EFun (f, es) -> (
      match f with
      | FMax | FMin -> (
          ...
          | [ e1; e2 ] ->
              ...
              if not (teq e1_type TInt) then
                ...
                let e2_type = typecheck_exp ctx e2 in
                if not (teq e2_type TInt) then
                  typ_err e2.eloc (typ_mismatch_err TInt e2_type)
                else TInt
          | _ ->
              let msg =
                Printf.sprintf "function expected 2 arguments, got %d"
                  (List.length es)
              in
      ...
      | FSize -> (
          ...
          | _ ->
              let msg =
                Printf.sprintf "function expected 1 arguments, got %d"
                  (List.length es)
              in
              syn_err msg exp.eloc))
...
and typecheck_stmt ctx s =
  ...
  | SAssign (lh, e) -> (
      ...
      | LHArr (v, e') -> (
          ...
          | TArray t ->
              ...
              if not (teq e'_type TInt) then
                typ_err e'.eloc (typ_mismatch_err TInt e'_type)
              else if not (teq t e_type) then
                typ_err e.eloc (typ_mismatch_err t e_type)
              else ctx
          | _ ->
              typ_err e.eloc
                (Printf.sprintf "expected type array, got %s"
  ...
  | SIf (e, s1, s2) ->
      let e_type = typecheck_exp ctx e in
      if not (teq e_type TBool) then
        typ_err e.eloc (typ_mismatch_err TBool e_type)
      else
