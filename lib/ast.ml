...
type var = string
type typ = TInt | TBool | TArray of typ
...
type func = FMax | FMin | FSize
type value = VConst of const | VArr of const list
...
type lhs_ = LHVar of var | LHArr of var * exp
...
let string_of_pos p =
  Lexing.(Printf.sprintf "%d:%d" p.pos_lnum (p.pos_cnum - p.pos_bol))

let syn_err s (spos, epos) =
  Printf.printf "%s--%s: Syntax Error: %s\n" (string_of_pos spos)
    (string_of_pos epos) s;
  raise SyntaxError
